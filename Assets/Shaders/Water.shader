...
Shader "Test/2TScroll"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 _TintColor;
			float _WorldTiling, _WaveStrength, _ScrollTex1ScrollX, _ScrollTex1ScrollY, _ScrollTex2ScrollX, _ScrollTex2ScrollY;

			v2f vert(appdata input)
			{
				v2f output;

				float2 worldCoords = mul(unity_ObjectToWorld, input.position).xz / _WorldTiling.xx;

				output.uv.xy = TRANSFORM_TEX(worldCoords, _ScrollingTex1) + half2(_ScrollTex1ScrollX, _ScrollTex1ScrollY) * _Time.yy;
				output.uv.zw = TRANSFORM_TEX(worldCoords, _ScrollingTex2) + half2(_ScrollTex2ScrollX, _ScrollTex2ScrollY) * _Time.yy;

				output.position = input.position;
				output.position.y += tex2Dlod(_ScrollingTex2, float4(output.uv.zw, 0.0, 0.0)).r * _WaveStrength;
